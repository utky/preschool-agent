 ...
 * ページネーションを自動処理する
 */
export const fetchLetters = async (baseUrl, modifiedAfter) => {
    const results = [];
    let page = 1;
    while (true) {
 ...
 */
export const fetchAttachments = async (baseUrl, letterId) => {
    ...
    const response = await fetch(url.toString());
    if (!response.ok) {
        throw new Error(`添付ファイル取得エラー: letterId=${letterId} status=${response.status}`);
    }
 ...
 */
export const buildGcsPath = (media) => {
