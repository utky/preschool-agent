...
> | Google Calendar自動同期 | iCalダウンロード（抽出精度の問題でユーザーが個別選択して取り込む方式に変更） |

### 目標
PDFから予定を抽出し、iCalファイルとしてダウンロードしてカレンダーアプリに取り込める機能を実装。
抽出精度の課題から、全件自動同期ではなくユーザーが各予定を個別にダウンロードして取り込む方式を採用。
...
- `frontend/src/components/events/EventCard.tsx` - iCalダウンロードボタン追加
- `frontend/src/components/events/EventTable.tsx` - iCal列追加（同期列削除）

**主要な実装**:
1. **イベント一覧ページ**: カード形式レイアウト（日付・時刻・タイトル・説明・文書リンク）
   - 各カードに「iCalダウンロード」ボタン
   - クリックで `ical_content` 文字列からBlobを生成し `.ics` ファイルをダウンロード
...
5. 「iCalダウンロード」ボタンで `.ics` ファイルが取得でき、カレンダーアプリに取り込めることを確認
