...
- `dbt/models/marts/core/fct_calendar_sync_history.sql` - カレンダー登録履歴（fct_ プレフィックス付き）
...
2. **fct_calendar_sync_history.sql**: カレンダー登録履歴（バックエンドが直接 INSERT）
3. **fct_events_with_sync.sql**: `fct_events`＋`dim_documents`＋`fct_calendar_sync_history`を結合
   - 未来イベント OR 未同期イベントをフィルタ（ビジネスロジックをマートで完結）
4. **exp_api__events.sql**:
   - `fct_events_with_sync` から SELECT のみ（JOIN・フィルタなし）
